...
freely available paper on [PLOS computational biology](http://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1003285). As [I'm currently very interested]({% post_url 2017-01-19-reproducible_analysis_my_principles %}) on the subject of reproducible data analysis, it caught my attention.

In this post, I will comment on this ten rules and the possible implementation in R with my point of view of epidemiologist interested in healthcare data reuse. I will also check if [my workflow]({% post_url 2017-01-21-implementation_basic_reproductible_workflow%}) comply with these rules.

For those who are in a hurry, I [summarise these rules and possible implementation in R in a table at the end of this post](#summary).

# Introduction

The author of this paper, Geir Kjetil Sandve, is [ assistant-professor of Norwegian](http://www.mn.uio.no/ifi/english/people/aca/geirksa/index.html) in a biomedical informatics lab. It is possible he wrote these rules with R in mind because he cites R in rule 7. However, biomedical informatics is quite far from my practice and he seems to use other tools. For example, they have dedicated integrated framework allowing reproducibility. 
...
In other words, one report = one script. But there is an extra step because the ability to reproduce a result may rely on previous steps. Then Sandve advice is to note the exact sequence of steps or even better, to allow the direct execution of everything. I implemented this with the "[One report file = one R markdown file]({% post_url 2017-01-21-implementation_basic_reproductible_workflow %}#report_equal_rmarkdown)" rule and the "[one R script to rule them all]({% post_url 2017-01-21-implementation_basic_reproductible_workflow %}#run_all)". Another interesting proposition is to use Make files in order to link each part, especially when relying on different software. I already use this to produce my MPH dissertation PDF and I found this perfect in this case. Yet, there is a major drawback: makefiles are not platform-agnostic.
...
Just use a version control system. If you are using R with Rstudio, git is the natural choice. It is well integrated and allow to interact with GitHub that tends to be (for good or bad) an unofficial package repository. The learning curve of this software is not steep compared to R.
...
In my case, my "Public" are the people who ask me to analyse their data. Then I just send to them the whole project folder and explain them quickly were to find the reports (in the */reports/* subdirectory) and plots for their papers (in the *plots/* directory). It's straightforward and doesn't add any extra step.
...
This is not exactly the spirit of Sandve advice. He advocate for data and scripts sharing to everyone for the sake of science transparency. The project I'm working on are not always for papers (*e.g.* analysis of an epidemic in my healthcare facility) and the aim is not to share it to the world. But more and more medical paper ask for this material and I will be happy to provide it if I have the opportunity to submit a paper.
...
All the 10 rules are easily reachable with R. Just by using R itself, the *rmarkdown* workflow and some organisational rules cover most of these rules.
